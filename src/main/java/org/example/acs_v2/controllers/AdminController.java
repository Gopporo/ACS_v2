...
public class AdminController {
    ...
    @ResponseBody
    public Map<String, Object> adminFingerprintStatus(@RequestParam Long userId) {
        User user = userService.getById(userId);
        ...
        if (state == TcpFingerprintServer.RegistrationState.IDLE) {
            // Если процесс сейчас не идёт, а отпечаток уже есть — считаем это "готово",
            // но НЕ используем это как индикатор прогресса для перезаписи.
